...
from utils.FLIR import FlirBFS
...
try:
    import tornado
    print("Tornado module imported successfully")
...
try:
    ...
    print("Error: pycoral module not found. Ensure you have installed pycoral correctly.")
    sys.exit(1)

try:
    import RPi.GPIO as GPIO
except ImportError:
    print("Error: RPi.GPIO module not found. Ensure you have installed RPi.GPIO correctly.")
    sys.exit(1)
...
def capture_flir_camera(on_new_frame, interpreter, mean, sliding_window, send_over_ws, cam_sockets):
    from utils import FLIR
    print("Initializing FLIR Camera")
    cam = FLIR.FlirBFS(on_new_frame=on_new_frame,
...
if __name__ == '__main__':
    ...
    if args.flir:
        capture_flir_camera(
            partial(on_new_frame, interpreter=interpreter, mean=mean, sliding_window=sliding_window,
